...
- 站外每日发现（重点板块）：先 1-2 句总览——分别说 Slickdeals（`offsite.summary_by_source.slickdeals`：品牌数 `brand_count`、Deal 数 `deal_count`、价位段 `price_min`-`price_max`）与 Hip2save（`offsite.summary_by_source.hip2save` 同上），并点出全站最低价（`offsite.price_range.lowest_price_deal` 的 `brand` 与 `price`）。随后给**两张紧凑 Markdown 表格**，均只列**主要竞品**（监控品牌）的 Deal、按品牌归类：
  - **Slickdeals**（数据源 `offsite.slickdeals_competitor_deals`），列「品牌 | Deal | 折扣 | Frontpage | 👍 | 💬 | 链接」：`title` 取简短；`discount_pct` 写「N%」、空写「—」；`is_frontpage` true→「是」/ false→「否」/ 空→「未知」；`thumbs_up`、`comments_count` 写数字、空写「—」；`url` 用 Markdown 链接（文案如「查看」）。
  - **Hip2save**（数据源 `offsite.hip2save_competitor_deals`），列「品牌 | 帖子 | 折扣 | 💬 | 发布日 | 链接」：`posted_at` 只取日期。
  - 某来源无主要竞品 Deal，写「今日 Slickdeals/Hip2save 暂无主要竞品 Deal」，不要编造行。逐行表里不放价格（已在总览）。非监控品牌只体现在总览计数，不进表。
