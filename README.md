...
站外报告会丢弃价格低于 `ANALYSIS_MIN_OFFSITE_PRICE`（默认 $5）的 Deal，并对 fan 类目排除玩具/一元区/食品合集语境（如 bullseye、playground、blaster、bubble fan、peeps、cupcake、hostess）、粉丝/受众语境（如 Dunkin' Fans 等促销贴）与家用大风扇（ceiling/tower/pedestal/box 等）。Analyzer 出报告前会对站外 Deal 再做一次类目相关性复核，已入库的历史噪音也会被即时滤除。
